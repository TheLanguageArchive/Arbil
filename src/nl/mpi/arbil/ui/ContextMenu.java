...
//        cellTableDivider = new JSeparator();
        ...
        deleteFieldMenuItem.addActionListener(new java.awt.event.ActionListener() {
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                try {
                    ArbilField[] selectedFields = currentTable.getSelectedFields();
...
//                            GuiHelper.linorgBugCatcher.logError(new Exception("deleteFromDomViaId"));
        ...
        searchReplaceMenuItem.addActionListener(new java.awt.event.ActionListener() {
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                try {
                    ((ArbilSplitPanel) currentTable.getParent().getParent().getParent().getParent()).showSearchPane();
                } catch (Exception ex) {
                    GuiHelper.linorgBugCatcher.logError(ex);
                }
            }
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                try {
                    ...
                    if (filteredNodes.size() > 0) {
                        ArbilWindowManager.getSingleInstance().openFloatingTableOnce(filteredNodes.toArray(new ImdiTreeObject[]{}), null);
                    }
                } catch (Exception ex) {
                    GuiHelper.linorgBugCatcher.logError(ex);
                }
...
//                        LinorgWindowManager.getSingleInstance().openDiffWindow(currentNode);
    ...
    private void searchSubnodesMenuItemActionPerformed(java.awt.event.ActionEvent evt) {
        ArbilWindowManager.getSingleInstance().openSearchTable(((ImdiTree) TreeHelper.getSingleInstance().arbilTreePanel.localCorpusTree).getSelectedNodes(), "Search");
    }

    private void searchRemoteSubnodesMenuItemActionPerformed(java.awt.event.ActionEvent evt) {
        ArbilWindowManager.getSingleInstance().openSearchTable(((ImdiTree) TreeHelper.getSingleInstance().arbilTreePanel.remoteCorpusTree).getSelectedNodes(), "Search Remote Corpus");
    }
    ...
    private void setupTableMenuItems() {
        if (currentTable != null) {
            if (currentTable.getSelectedRow() != -1) {
                ...
                ArbilField[] currentSelection = currentTable.getSelectedFields();
            ...
            if (currentTable.getParent().getParent().getParent().getParent() instanceof ArbilSplitPanel) {
