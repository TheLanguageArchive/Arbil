...
public class MimeHashQueue {
	...
	if (fileType == null) {
	    ...
	    try {
		configFile = LinorgSessionStorage.getSingleInstance().getTypeCheckerConfig();
	    } catch (Exception ex) {
		GuiHelper.linorgBugCatcher.logError("Error while trying to retrieve file checker configuration", ex);
	    }
	    if (configFile != null) {
