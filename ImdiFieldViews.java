...
public class ImdiFieldViews {
    ...
    // string arrays of descriptions for the available imdi fields
    private Hashtable masterImdiFieldDescription;
    // string arrays of aliases for the available imdi fields
    //String[] masterImdiFieldAliases;
    ...
    public DefaultTableModel getImdiFieldViewTableModel(Object currentView) {
        ...
        // we want a table model even if it has no rows
        if (currentView != null) {
            // add the show only fields
            for (Enumeration showOnlyColumnNames = ((FieldView) currentView).getShowOnlyColumns(); showOnlyColumnNames.hasMoreElements();) {
                String currentFieldName = showOnlyColumnNames.nextElement().toString();
                returnTableModel.addRow(new Object[]{currentFieldName, getMasterImdiFieldDescription(currentFieldName), true, false, false});
            }
            // add the hidden fields
            for (Enumeration showHiddenColumnNames = ((FieldView) currentView).getHiddenColumns(); showHiddenColumnNames.hasMoreElements();) {
                String currentFieldName = showHiddenColumnNames.nextElement().toString();
                returnTableModel.addRow(new Object[]{currentFieldName, getMasterImdiFieldDescription(currentFieldName), false, true, false});
            }
            // add the hidden fields
            for (Enumeration knownColumnNames = ((FieldView) currentView).getKnownColumns(); knownColumnNames.hasMoreElements();) {
                ...
                returnTableModel.addRow(new Object[]{currentFieldName, getMasterImdiFieldDescription(currentFieldName), false, false, false});
            }
        }
    ...
    private void loadImdiFieldViews() {
        ...
        try {
            savedFieldViews = (Hashtable) linorgSessionStorage.loadObject("savedFieldViews");
            currentGlobalViewName = (String) linorgSessionStorage.loadObject("currentGlobalViewName");
        } catch (Exception ex) {
        ...
        // set up / load the desciption text for known fields 
        masterImdiFieldDescription = new Hashtable();
        //masterImdiFieldAliases = new String[masterImdiFieldNames.length];
        masterImdiFieldDescription.put(".name", "description for name fields");
        masterImdiFieldDescription.put("session.name", "description for session name fields");
        masterImdiFieldDescription.put("corpus.name", "description for corpus name fields");
    }

    public void saveViewsToFile() {
        try {
            linorgSessionStorage.saveObject(savedFieldViews, "savedFieldViews");
            linorgSessionStorage.saveObject(currentGlobalViewName, "currentGlobalViewName");
        } catch (Exception ex) {
    ...
    public int getMasterImdiFieldCount() {
        return masterImdiFieldNames.length;
    }

    public String getMasterImdiFieldName(int fieldIndex) {
        return masterImdiFieldNames[fieldIndex];
    }

    public String getMasterImdiFieldDescription(String fieldName) {
        Object fieldDescription = masterImdiFieldDescription.get(fieldName);
...
class FieldView implements Serializable {
    ...
    Vector hiddenColumns = new Vector();
    Vector showOnlyColumns = new Vector();
...
//    hiddenColumns, showOnlyColumns, knownColumns, alwaysShowColumns
    ...
    public Enumeration getShowOnlyColumns() {
        return showOnlyColumns.elements();
    }

    public Enumeration getHiddenColumns() {
        return hiddenColumns.elements();
    }
}
