                    ...
                    public void actionPerformed(java.awt.event.ActionEvent evt) {
                        try {
                            ...
                            if (selectedFiles != null && selectedFiles.length > 0) {
                                ...
                                changeCacheDirectory(selectedFiles[0], true);
                            }
                        } catch (Exception ex) {
                            BugCatcherManager.getBugCatcher().logError(ex);
                        }
...
//    }
    ...
    public void changeCacheDirectory(File preferedCacheDirectory, boolean moveFiles) {
        File fromDirectory = sessionStorage.getProjectWorkingDirectory();
        if (!preferedCacheDirectory.getAbsolutePath().contains(sessionStorage.getProjectDirectoryName()) && !preferedCacheDirectory.getAbsolutePath().contains(".arbil/imdicache") && !sessionStorage.getProjectWorkingDirectory().getAbsolutePath().contains(".linorg/imdicache")) {
            preferedCacheDirectory = new File(preferedCacheDirectory, sessionStorage.getProjectDirectoryName());
        }
        ...
        if (!moveFiles || JOptionPane.YES_OPTION == windowManager.showDialogBox(
            ...
            if (moveFiles) {
                ...
                sessionStorage.saveString("cacheDirectory", preferedCacheDirectory.getAbsolutePath());
            }
        }
    }
// Move the storage directory and change the local corpus tree links to the new directory.
    ...
    private void changeStorageDirectory(File fromDirectory, File toDirectory) {
        ...
        if (!success) {
            if (JOptionPane.YES_OPTION == windowManager.showDialogBox(
                ...
                sessionStorage.saveString("cacheDirectory", toDirectory.getAbsolutePath());
                sessionStorage.clearProjectWorkingDirectory();
