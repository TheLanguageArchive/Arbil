...
/**
 * Document : ArbilDragDrop Created on : Tue Sep 09 15:02:56 CEST 2008
 *
            ...
            if (dropTargetDataNode != null) {
                ...
                if (dropTargetDataNode.getParentDomNode().isCmdiMetaDataNode() && !selectionDraggedFromLocalCorpus
                    ...
                    if (selectionContainsArchivableLocalFile == true
                        ...
                        new MetadataBuilder().requestAddNodes(ArbilFavourites.getSingleInstance(), windowManager, dropTargetDataNode, widgets.getString("RESOURCE"), draggedArbilNodes);
                        return true;
                    }
                }
            }
                        ...
                        if (!draggedIntoSelf) {
                            if (currentNode.isFavorite()) {
                                ...
                                if (dropTargetNode instanceof ArbilRootNode) {
                                    ...
                                    new MetadataBuilder().requestAddRootNode(ArbilFavourites.getSingleInstance(), windowManager, currentNode, currentNode.toString());
                                } else if (dropTargetDataNode != null) {
                                    new MetadataBuilder().requestAddNode(ArbilFavourites.getSingleInstance(), windowManager, dropTargetDataNode, currentNode.toString(), currentNode);
                                } else {
                ...
                if (dropTargetDataNode.isCorpus()) {
                    ...
                    if (MetadataReader.getSingleInstance().nodeCanExistInNode(dropTargetDataNode, currentNode)) {
                        try {
                            ...
                            new MetadataBuilder().addNodes(ArbilFavourites.getSingleInstance(), windowManager, dropTargetDataNode, new String[]{currentNode.toString()}, new ArbilDataNode[]{currentNode});
                            addNodeResult = true;
                        } catch (ArbilMetadataException ex) {
                            BugCatcherManager.getBugCatcher().logError(ex);
